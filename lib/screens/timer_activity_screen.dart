...
class _TimerActivityScreenState extends State<TimerActivityScreen> {
  ...
  void initState() {
    super.initState();
    _loadCounter();
    retrieveActivityTime(widget.activityID); // Retrieve activity time when the screen is initialized because the activityID is passed to the TimerActivityScreen widget
  }

  Future<void> _loadCounter() async {
    // Load initial counter value from Firestore or set a default value
    setState(() {
      _counter = 0; // Set your initial counter value here
    });
  }
  ...
  Future<void> _startTimer() async {
    ...
    // Save timer log details (timerLogID and startTime)
  ...
  void _resumeTimer() async {
    ...
    if (_timerLogID == null) { // Create timerLogID if it's the first time the timer is resumed after starting it
      int timerLogCount = await DatabaseMethods().getTimerLogCount(widget.activityID) + 1;
      _timerLogID = "T${timerLogCount.toString().padLeft(4, '0')}";
    }
